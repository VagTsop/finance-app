import { Injectable, computed, signal } from '@angular/core';

import type {
  Balance,
  BudgetSummary,
  FinanceData,
  PotSummary,
  Transaction
} from '../models';

const DATA: FinanceData = {
  balance: {
    current: 4836,
    income: 3814.25,
    expenses: 1700.5
  },
  transactions: [
    {
      id: '1',
      avatar: 'assets/avatars/avatar_4.jpg',
      name: 'Emma Richardson',
      category: 'General',
      date: '2024-08-19T14:23:11Z',
      amount: 75.5,
      recurring: false
    },
    {
      id: '2',
      avatar: 'assets/avatars/savory-bites-bistro.jpg',
      name: 'Savory Bites Bistro',
      category: 'Dining Out',
      date: '2024-08-19T20:23:11Z',
      amount: -55.5,
      recurring: false
    },
    {
      id: '3',
      avatar: 'assets/avatars/avatar_0.jpg',
      name: 'Daniel Carter',
      category: 'General',
      date: '2024-08-18T09:45:32Z',
      amount: -42.3,
      recurring: false
    },
    {
      id: '4',
      avatar: 'assets/avatars/avatar_3.jpg',
      name: 'Sun Park',
      category: 'General',
      date: '2024-08-17T16:12:05Z',
      amount: 120,
      recurring: false
    },
    {
      id: '5',
      avatar: 'assets/avatars/urban-services-hub.jpg',
      name: 'Urban Services Hub',
      category: 'General',
      date: '2024-08-17T21:08:09Z',
      amount: -65,
      recurring: false
    },
    {
      id: '6',
      avatar: 'assets/avatars/avatar_1.jpg',
      name: 'Liam Hughes',
      category: 'Groceries',
      date: '2024-08-15T18:20:33Z',
      amount: 65.75,
      recurring: false
    },
    {
      id: '7',
      avatar: 'assets/avatars/avatar_2.jpg',
      name: 'Lily Ramirez',
      category: 'General',
      date: '2024-08-14T13:05:27Z',
      amount: 50,
      recurring: false
    },
    {
      id: '8',
      avatar: 'assets/avatars/avatar_2.jpg',
      name: 'Ethan Clark',
      category: 'Dining Out',
      date: '2024-08-13T20:15:59Z',
      amount: -32.5,
      recurring: false
    },
    {
      id: '9',
      avatar: 'assets/avatars/avatar_0.jpg',
      name: 'James Thompson',
      category: 'Entertainment',
      date: '2024-08-11T15:45:38Z',
      amount: -5,
      recurring: false
    },
    {
      id: '10',
      avatar: 'assets/avatars/pixel-playground.jpg',
      name: 'Pixel Playground',
      category: 'Entertainment',
      date: '2024-08-11T18:45:38Z',
      amount: -10,
      recurring: true
    },
    {
      id: '11',
      avatar: 'assets/avatars/avatar_1.jpg',
      name: 'Ella Phillips',
      category: 'Dining Out',
      date: '2024-08-10T19:22:51Z',
      amount: -45,
      recurring: false
    },
    {
      id: '12',
      avatar: 'assets/avatars/avatar_2.jpg',
      name: 'Sofia Peterson',
      category: 'Transportation',
      date: '2024-08-08T08:55:17Z',
      amount: -15,
      recurring: false
    },
    {
      id: '13',
      avatar: 'assets/avatars/avatar_3.jpg',
      name: 'Mason Martinez',
      category: 'Lifestyle',
      date: '2024-08-07T17:40:29Z',
      amount: -35.25,
      recurring: false
    },
    {
      id: '14',
      avatar: 'assets/avatars/green-plate-eatery.jpg',
      name: 'Green Plate Eatery',
      category: 'Groceries',
      date: '2024-08-06T08:25:44Z',
      amount: -78.5,
      recurring: false
    },
    {
      id: '15',
      avatar: 'assets/avatars/avatar_1.jpg',
      name: 'Sebastian Cook',
      category: 'Transportation',
      date: '2024-08-06T10:05:44Z',
      amount: -22.5,
      recurring: false
    },
    {
      id: '16',
      avatar: 'assets/avatars/avatar_4.jpg',
      name: 'William Harris',
      category: 'Personal Care',
      date: '2024-08-05T14:30:56Z',
      amount: -10,
      recurring: false
    },
    {
      id: '17',
      avatar: 'assets/avatars/elevate-education.jpg',
      name: 'Elevate Education',
      category: 'Education',
      date: '2024-08-04T11:15:22Z',
      amount: -50,
      recurring: true
    },
    {
      id: '18',
      avatar: 'assets/avatars/serenity-spa-and-wellness.jpg',
      name: 'Serenity Spa & Wellness',
      category: 'Personal Care',
      date: '2024-08-03T14:00:37Z',
      amount: -30,
      recurring: true
    },
    {
      id: '19',
      avatar: 'assets/avatars/spark-electric-solutions.jpg',
      name: 'Spark Electric Solutions',
      category: 'Bills',
      date: '2024-08-02T09:25:11Z',
      amount: -100,
      recurring: true
    },
    {
      id: '20',
      avatar: 'assets/avatars/avatar_0.jpg',
      name: 'Rina Sato',
      category: 'Bills',
      date: '2024-08-02T13:31:11Z',
      amount: -50,
      recurring: false
    },
    {
      id: '21',
      avatar: 'assets/avatars/swift-ride-share.jpg',
      name: 'Swift Ride Share',
      category: 'Transportation',
      date: '2024-08-01T18:40:33Z',
      amount: -18.75,
      recurring: false
    },
    {
      id: '22',
      avatar: 'assets/avatars/aqua-flow-utilities.jpg',
      name: 'Aqua Flow Utilities',
      category: 'Bills',
      date: '2024-07-30T13:20:14Z',
      amount: -100,
      recurring: true
    },
    {
      id: '23',
      avatar: 'assets/avatars/ecofuel-energy.jpg',
      name: 'EcoFuel Energy',
      category: 'Bills',
      date: '2024-07-29T11:55:29Z',
      amount: -35,
      recurring: true
    },
    {
      id: '24',
      avatar: 'assets/avatars/avatar_0.jpg',
      name: 'Yuna Kim',
      category: 'Dining Out',
      date: '2024-07-29T13:51:29Z',
      amount: -28.5,
      recurring: false
    },
    {
      id: '25',
      avatar: 'assets/avatars/flavor-fiesta.jpg',
      name: 'Flavor Fiesta',
      category: 'Dining Out',
      date: '2024-07-27T20:15:06Z',
      amount: -42.75,
      recurring: false
    },
    {
      id: '26',
      avatar: 'assets/avatars/avatar_3.jpg',
      name: 'Harper Edwards',
      category: 'Shopping',
      date: '2024-07-26T09:43:23Z',
      amount: -89.99,
      recurring: false
    },
    {
      id: '27',
      avatar: 'assets/avatars/buzz-marketing-group.jpg',
      name: 'Buzz Marketing Group',
      category: 'General',
      date: '2024-07-26T14:40:23Z',
      amount: 3358,
      recurring: false
    },
    {
      id: '28',
      avatar: 'assets/avatars/technova-innovations.jpg',
      name: 'TechNova Innovations',
      category: 'Shopping',
      date: '2024-07-25T16:25:37Z',
      amount: -29.99,
      recurring: false
    },
    {
      id: '29',
      avatar: 'assets/avatars/bytewise.jpg',
      name: 'ByteWise',
      category: 'Lifestyle',
      date: '2024-07-23T09:35:14Z',
      amount: -49.99,
      recurring: true
    },
    {
      id: '30',
      avatar: 'assets/avatars/avatar_4.jpg',
      name: 'Nimbus Data Storage',
      category: 'Bills',
      date: '2024-07-21T10:05:42Z',
      amount: -9.99,
      recurring: true
    },
    {
      id: '31',
      avatar: 'assets/avatars/avatar_4.jpg',
      name: 'Emma Richardson',
      category: 'General',
      date: '2024-07-20T17:30:55Z',
      amount: -25,
      recurring: false
    },
    {
      id: '32',
      avatar: 'assets/avatars/avatar_0.jpg',
      name: 'Daniel Carter',
      category: 'General',
      date: '2024-07-19T12:45:09Z',
      amount: 50,
      recurring: false
    }
  ],
  budgets: [
    {
      id: '1',
      category: 'Entertainment',
      maximum: 50,
      theme: '#277C78'
    },
    {
      id: '2',
      category: 'Bills',
      maximum: 750,
      theme: '#82C9D7'
    },
    {
      id: '3',
      category: 'Dining Out',
      maximum: 75,
      theme: '#F2CDAC'
    },
    {
      id: '4',
      category: 'Personal Care',
      maximum: 100,
      theme: '#626070'
    }
  ],
  pots: [
    {
      id: '1',
      name: 'Savings',
      target: 2000,
      total: 159,
      theme: '#277C78'
    },
    {
      id: '2',
      name: 'Concert Ticket',
      target: 150,
      total: 110,
      theme: '#626070'
    },
    {
      id: '3',
      name: 'Gift',
      target: 60,
      total: 40,
      theme: '#82C9D7'
    },
    {
      id: '4',
      name: 'New Laptop',
      target: 1000,
      total: 10,
      theme: '#F2CDAC'
    },
    {
      id: '5',
      name: 'Holiday',
      target: 1440,
      total: 531,
      theme: '#826CB0'
    }
  ]
};

@Injectable({ providedIn: 'root' })
export class FinanceDataService {
  private readonly state = signal(DATA);

  readonly balance = computed<Balance>(() => this.state().balance);
  readonly transactions = computed<Transaction[]>(() =>
    [...this.state().transactions].sort((a, b) =>
      new Date(b.date).getTime() - new Date(a.date).getTime()
    )
  );

  readonly budgets = computed<BudgetSummary[]>(() => {
    const totals = new Map<string, number>();

    for (const tx of this.state().transactions) {
      if (tx.amount < 0) {
        const current = totals.get(tx.category) ?? 0;
        totals.set(tx.category, current + Math.abs(tx.amount));
      }
    }

    return this.state().budgets.map((budget) => {
      const spent = totals.get(budget.category) ?? 0;
      const remaining = Math.max(0, budget.maximum - spent);
      const percentage = budget.maximum === 0 ? 0 : Math.min(100, (spent / budget.maximum) * 100);

      return {
        ...budget,
        spent,
        remaining,
        percentage: Math.round(percentage)
      } satisfies BudgetSummary;
    });
  });

  readonly pots = computed<PotSummary[]>(() =>
    this.state().pots.map((pot) => ({
      ...pot,
      progress: pot.target === 0 ? 0 : Math.min(100, Math.round((pot.total / pot.target) * 100))
    }))
  );

  readonly recurringBills = computed<Transaction[]>(() =>
    this.transactions().filter((tx) => tx.recurring || tx.category === 'Bills')
  );

  getRecentTransactions(limit = 5): Transaction[] {
    return this.transactions().slice(0, limit);
  }

  getTotalRecurring(): number {
    return this.recurringBills().reduce((sum, bill) => sum + Math.abs(bill.amount), 0);
  }

  getCategoryTotals(): { category: string; total: number }[] {
    const totals = new Map<string, number>();

    for (const tx of this.state().transactions) {
      const current = totals.get(tx.category) ?? 0;
      totals.set(tx.category, current + tx.amount);
    }

    return [...totals.entries()].map(([category, total]) => ({
      category,
      total
    }));
  }
}
